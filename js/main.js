...
//	slick slider
  ...
  slickJQ(".main__slider img").mouseup(function() {

    var current = slickJQ('.main__slider').slick('getSlick').getCurrent();
    var clicked = slickJQ(this).attr('data-slick-index');

    if (clicked >= current) {

      slickJQ('.main__slider').slick('getSlick').changeSlide({
        data: {
          message: 'next'
        }
      });
...
let teacherContainer = document.querySelector('.teachers__container');
let teacherWrapp = teachers.querySelectorAll('.teacher__wrapp');

  let arrowLeft = teachers.querySelector('.arrow-left');
  let arrowRight = teachers.querySelector('.arrow-right');
  ...
  let position = 0;
   ...
   arrowLeft.addEventListener('click', () => {
    ...
    teacherContainer.style.marginLeft = position + 'px';
   ...
   arrowRight.addEventListener('click', () => {
    position = Math.max(position - width * count, -width * (teacherWrapp.length - count));
    teacherContainer.style.marginLeft = position + 'px';
